...
% In v2, the length of a arm at extrema is compared with average arm
% length. And then use the discrepency to determine event.

% In v3, the length of a arm is compared with the length at previous extremum
% Then the change in length bewteen two successive extrema is valued against 
% variance of arm length.

function [occurance, index]=find_scatter_v3(pkh,pkl,lnh,lnl)
    pg=1;
     index=zeros(100,1);
