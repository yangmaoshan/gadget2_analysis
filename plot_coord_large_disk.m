...
%l='http://www.illustris-project.org/api/Illustris-1/snapshots/';
 ...
 % p0=[0;0;0];
  % v0=[0;0;0];
   ...
   V1_ini= double(h5read(g2,'/PartType0/Velocities/'));
   V2_ini= double(h5read(g2,'/PartType2/Velocities/'));
  
   
   C1_ini =double( h5read(g2,'/PartType0/Coordinates/'));
   ...
   C1=C1_ini(1:3,select);
   
   V1=V1_ini(1:3,select);
   p0=mean(C2,2);
   v0=mean(V2,2);
 ...
 %  la1=[ 'id='  num2str(id) ',   '  'nstar='   num2str(b(3,ij)) ',   '  'condi=' num2str(b(2,ij)) ];
  % la2=[ 'relax time='   num2str(b(4,ij)) ',   '  'relax time2='   num2str(b(5,ij)) ',   ' ];
   ...
   la3=[ 'snapshot_v2_select__title_3=' num2str(ij) '.png'] ;
     xx=transpose(C2(1,:));
yy=transpose(C2(2,:));
zz=transpose(C2(3,:));
tabl=table(xx,yy,zz);
...
gsize_s=20;%gsize_s=25;gsize_s=20;
gsize_g=20;%gsize_g=30;
...
gra5.MarkerEdgeAlpha = 0.1;
gra6.MarkerEdgeAlpha = 0.1;
%{
gra5=scatter(ax5,C1(1,:),C1(2,:),10,'.');
